...
void task_init(void) {
  ...
  for(int i = 0; i < LAB_TEST_NUM; ++i) {
    ...
    create_mapping((uint64_t *)root_pt, 0xffffffc000000000, 0x80000000, 16*1024*1024, PTE_V | PTE_R | PTE_W | PTE_X);
    ...
    create_mapping((uint64_t *)root_pt, 0x80000000, 0x80000000, 16*1024*1024, PTE_V | PTE_R | PTE_W | PTE_X);
    // 9. 修改对内核空间不同 section 所在页属性的设置，完成对不同section的保护，其中text段的权限为 r-x, rodata 段为 r--, 其他段为 rw-，注意上述两个映射都需要做保护。
    create_mapping((uint64_t *)root_pt,(uint64_t)&text_start , PHYSICAL_ADDR((uint64_t)&text_start), (uint64_t)&rodata_start-(uint64_t)&text_start, PTE_R|PTE_X);
    create_mapping((uint64_t *)root_pt, (uint64_t)&rodata_start, PHYSICAL_ADDR((uint64_t)&rodata_start), (uint64_t)&data_start-(uint64_t)&rodata_start, PTE_R);           
    create_mapping((uint64_t *)root_pt, (uint64_t)&data_start, PHYSICAL_ADDR((uint64_t)&data_start), (uint64_t)&user_program_start - (uint64_t)&data_start, PTE_R|PTE_W);
    // 10. 将必要的硬件地址（如 0x10000000 为起始地址的 UART ）进行等值映射 ( 可以映射连续 1MB 大小 )，无偏移，PTE_V | PTE_R | PTE_W 为映射的读写权限
    create_mapping((uint64_t *)root_pt, 0x10000000, 0x10000000, 1*1024*1024, PTE_V | PTE_R | PTE_W);
